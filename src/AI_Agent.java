...
public abstract class AI_Agent extends Agent {
    ...
    public int calculateNextStep(int position, boolean type, int maxExpands) {
        ...
        if (!Main.isBonus)
            ...
            temp_s = new BonusState(position, p_arr, peopleInCar, time, f_func, -1, copy(Main.vertexMatrix),
                    Main.agents[0].position);
        else
            temp_s = new BonusState(position, p_arr, peopleInCar, time, f_func, -1, copy(Main.vertexMatrix),
                    Main.agents[1].position);
        ...
        while (true) {
            ...
            if (Main.isBonus) {
                ...
                if (Main.agents[0].agentType == 3) {
                    VandalState vs = vandalStep(((BonusState) st.get(temp_n.index)).copyArr, Main.agents[0].position,
                            time);
                    ...
                    VandalState vs = vandalStep(((BonusState) st.get(temp_n.index)).copyArr, Main.agents[1].position,
                            time);
                    ((BonusState) st.get(temp_n.index)).copyArr = vs.arr;
                    ((BonusState) st.get(temp_n.index)).vandalVertex = vs.v2;
                    ((BonusState) st.get(temp_n.index)).time = vs.timeForAction;
    ...
    public void expand(Node n, State s, boolean type) {
        ...
        for (int i = 0; i < Main.vertexMatrix.length; i++) {
            ...
            if (!Main.isBonus) {
                ...
                if (vertex < i && ((BonusState) s).copyArr[vertex][i] > 0) {
                    edgeWeight = DijkstraAlgorithm.cTime(((BonusState) s).copyArr[vertex][i], s.peopleInCar,
                            Main.k_parameter);
                    numOfNeighbors++;
                } else if (vertex > i && ((BonusState) s).copyArr[i][vertex] > 0) {
                    edgeWeight = DijkstraAlgorithm.cTime(((BonusState) s).copyArr[i][vertex], s.peopleInCar,
                            Main.k_parameter);
                    numOfNeighbors++;
                } else
                    continue;
            ...
            // bonus
            if (!Main.isBonus) {
                f_func = HeuristicCalculator.calculateF(i, peopleArr, sumOfPeople, s.time + edgeWeight,
                        Main.mostHeavyEdge, type);
                temp_s = new State(i, peopleArr, sumOfPeople, s.time + edgeWeight, f_func, n.index);
            } else {
                f_func = BonusFunc.calculateF(i, peopleArr, sumOfPeople, s.time + edgeWeight, Main.mostHeavyEdge, type,
                        ((BonusState) s).copyArr);
                temp_s = new BonusState(i, peopleArr, sumOfPeople, s.time + edgeWeight, f_func, n.index,
                        ((BonusState) s).copyArr.clone(), ((BonusState) s).vandalVertex);
            }
            st.add(temp_s);
            temp_n = new Node(st.size() - 1, f_func);
            mh.insert(temp_n);
        ...
        if (numOfNeighbors == 0) {
            if (!Main.isBonus) {
                f_func = HeuristicCalculator.calculateF(vertex, s.peopleArr, s.peopleInCar, s.time + 1,
                        Main.mostHeavyEdge, type);
                temp_s = new State(vertex, s.peopleArr, s.peopleInCar, s.time + 1, f_func, vertex);

            } else {
                f_func = BonusFunc.calculateF(vertex, s.peopleArr, s.peopleInCar, s.time + 1, Main.mostHeavyEdge, type,
                        ((BonusState) s).copyArr);
                temp_s = new BonusState(vertex, s.peopleArr, s.peopleInCar, s.time + 1, f_func, vertex,
                        ((BonusState) s).copyArr.clone(), ((BonusState) s).vandalVertex);
            }
...
class BonusState extends State {
    ...
    public BonusState(int vertex, int[] peopleArr, int peopleInCar, double time, double heuristic, int prev,
            double arr[][], int vandalVertex) {
